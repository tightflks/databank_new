    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 font-sans" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 sm:p-8 relative max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {trialEndedFor ? (
          ...
          <p className="text-sm text-db-muted mb-6">{mode === 'signup' ? 'Start your free 30-day trial. No credit card needed.' : 'Sign in with your Databank account.'}</p>
        )}
        ...
        <p className="text-xs text-gray-400 text-center mt-4">
          {mode === 'signup' ? (
            <>Already have an account? <button onClick={() => { setMode('login'); setError(null); }} className="text-db-navy font-semibold hover:underline">Sign in</button></>
          ) : (
            <>New here? <button onClick={() => { setMode('signup'); setError(null); }} className="text-db-navy font-semibold hover:underline">Start a free trial</button></>
          )}
        </p>
      </div>
    </div>
